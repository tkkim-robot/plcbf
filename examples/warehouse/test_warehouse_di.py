...
@description:
Test script for Warehouse Scenario with Double Integrator dynamics.
...
from examples.warehouse.controllers.nominal_di import (
    ...
    StopBackupController, RetraceBackupController
)
...
# Empirical DI rollout reach with the refreshed defaults is similar to quad3d.
...
DETECTED_GHOST_COLOR = '#f16d7a'
UNDETECTED_GHOST_COLOR = '#fbe3e8'
ROBOT_TRAJECTORY_COLOR = '#5b8ff9'
ROBOT_TRAJECTORY_LINEWIDTH = 1.6
ROBOT_TRAJECTORY_ALPHA = 0.5
PAPER_ARROW_SCALE_M = 3.2
PAPER_ARROW_MIN_SPEED = 1e-4
DEFAULT_PAPER_HALF_WINDOW_M = 25.0
...
def setup_test(
    ...
    robot_spec = {
        ...
        'radius': 1.0,
        'v_ref': 3.0,
        # Nominal/backup tuning mirrors the optimized quad3d warehouse scale.
        'nominal_Kp_v': 7.0,
        'nominal_K_lat': 1.2,
    ...
    # Nominal Controller
    nominal_ctrl_obj = WaypointFollower(
    ...
    # Python Backup Controller (for baselines and PCBF retrace fallback)
    if algo in ['backup_cbf', 'gatekeeper', 'mps', 'pcbf']:
        py_backup = RetraceBackupController(
            nominal_ctrl_obj,
        ...
        py_backup = StopBackupController(Kp_braking=robot_spec['stop_Kp_v'])
    ...
    if algo == 'pcbf':
        ...
        filter_algo = Gatekeeper(
            robot, robot_spec, dt=env.dt, backup_horizon=backup_horizon,
...
def run_simulation(args, scenario_ghosts=None):
    ...
    env, robot, nom_ctrl, shielding, robot_spec = setup_test(
        args.algo,
        args.level,
    ...
    if not args.no_render:
        ...
        if wps is not None and len(wps) > 0:
            wp_colors = np.tile(np.array([1.0, 0.78, 0.0, 1.0]), (len(wps), 1))
            ...
            def _update_waypoints(force=False):
                ...
                if not force and wp_state['last_idx'] == current_idx and wp_state['last_visited'] == visited:
                    return
                wp_colors[:] = [1.0, 0.78, 0.0, 1.0]
                if visited > 0:
                    wp_colors[:visited, 3] = 0.0
                if visited < len(wps):
                    wp_colors[current_idx] = [1.0, 0.45, 0.0, 1.0]
            ...
            for i, ghost in enumerate(env.ghosts):
                ...
                if ghost_velocity_arrows is not None and i < len(ghost_velocity_arrows):
                    ...
                    if speed > PAPER_ARROW_MIN_SPEED:
                        ...
                        ex = sx
                        ey = sy
                    ghost_velocity_arrows[i].set_positions((sx, sy), (ex, ey))
    ...
    if args.save and fig:
        save_dir = getattr(args, 'save_dir', None) or f"output/animations/warehouse_di_{args.algo}_lvl{args.level}"
    ...
    print(f"Starting DI {args.algo} Level {args.level}...")
    ...
    for step in range(max_steps):
        ...
        if step % 200 == 0:
            ...
            print(
                f"STEP[{step}]: Pos={current_state[:2]} | Vel={current_state[2:4]} | "
                f"GoalDist={dist_goal:.2f} | WP={nom_ctrl.wp_idx}/{len(nom_ctrl.waypoints)}"
            )
...
def run_sweep(args):
    ...
    for level in target_levels:
        for alg in algos:
            ...
            args_sim.animation_safety_margin = args.animation_safety_margin
            args_sim.alpha = args.alpha
            args_sim.plcbf_num_angle_policies = args.plcbf_num_angle_policies
            args_sim.timing_warmup_steps = args.timing_warmup_steps
            args_sim.max_steps = args.max_steps
...
def plot_results(data):
    ...
    plt.tight_layout()
    plt.savefig("warehouse_di_results.png")
    print("Saved warehouse_di_results.png")
